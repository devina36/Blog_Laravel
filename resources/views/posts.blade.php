...
@section('container')
    ...
    <div class="card mb-3">
        ...
        <div class="d-flex justify-content-center" style="max-height: 400px; overflow: hidden;">
            <img src="{{ asset('storage/' . $posts[0]->img) }}" class="img-fluid" alt="{{$posts[0]->category->name}}">
        </div>
        @else
            ...
            <div class="col-md-4 mb-3">
                <div class="card">
                    <div class="category-card">
                        <a href="/posts?category={{ $post->category->slug }}" class="category-card-text">{{$post->category->name}}</a>
                    </div>
                    @if ($post->img)
                    <img src="{{ asset('storage/' . $post->img) }}" class="img-fluid" alt="{{$post->category->name}}">
                    @else
